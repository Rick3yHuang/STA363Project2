...
Predicting the number of applicaitons received by a university per year is important for admission officers to allocate the incoming students' demands for housing, dining, and so on, which in turn can increase the accuracy of universities' budgets in these areas. In order to predict the number of applications, many aspects about the information of the universities are considered, $16$ variables like the acceptance rate, the number of undergraduate students, the student faculty ratio and so on are used to fit a model for prediction. In this project, four estimation models are fitted, which are least-square line regression model (LSLR model), ridge regression model (Ridge model), Lasso model, and the Elastic Net model. To compare the models, the accuracy for prediction of all four models is tested using the k-fold cross validation technique. The process and steps for building the models and the detail results are shown in the body paragraphs formal report. In conclusion, the Elastic Net model is suggest to the client to do the prediction job, since it has a lowest test RMSE which is $1624.236$.
...
The shrinkage technique I use here for the second model is the Ridge Regression. Improved from the LSLR model, the metric RSS plus a penalty term is minimized here to choose better estimates. To be specific, the metric we are minimizing here is expanded in the Formula \ref{eq:RidgeMetric}, where the $\lambda \geqslant 0$ is the tuning parameter and the $\lambda \hat{\beta}^{T} \hat{\beta}$ is the penalty term. By adding this penalty term, we can shrink the estimates and in turn lower the standard error of our model.
...
Like the modelling process in the ridge regression, the tuning variable $\lambda_{Lasso}$ are chosen among 0 to 1000 by 0.5 to generate a Lasso model with the least test MSE. The plot shown the relationship between the test MSE and the log Lambda is created in the Figure \ref{fig:LassoCV}. From the plot, the test MSE increases as $\lambda_{Lasso}$ increases, so $\lambda_{Lasso}$'s are not neccessary to be tested to determine an appropriate tuning parameter. As shown in the result of the 21-fold cross validation, the least test MSE is from the model with a tuning parameter of $19$, which results a test MSE of $3554176$.
...
When the model is fitted, several combinations of the $\lambda_{Elnet}$ and $\alpha$ are used to fit the model. For the same reason as before, the $21$-fold cross validation method is used to test the models different $\lambda_{Elnet}$ and $\alpha$'s, and the best model with the lowest RMSE is chosen to fit the final Elastic Net Model. 
