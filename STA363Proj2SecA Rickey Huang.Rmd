...
```{r lassoCV, fig.cap="\\label{fig:lassoCV}test MSE vs. log(Lamba) for the Lasso model"}
# Create the design matrix, using all variables we have
XD <- model.matrix(Apps ~ ., data = college)
# Doing the cross validation to choose the best tuning parameter
set.seed(100)
...
When the model is fitted, several combinations of the $\lambda_{Elnet}$ and $\alpha$ are used to fit the model. For the same reason, the $21$-fold cross validation method is used to test the models different $\lambda_{Elnet}$ and $\alpha$'s, and the best model with the lowest RMSE is chosen to fit the final Elastic Net Model. 
